...
def test_health_returns_503_when_database_is_unreachable(
    ...
    Fails at `ensure_connection`, which is where a genuinely unreachable database fails, rather
    than by swapping the connection object inside the view. That distinction matters: an earlier
    version of this test patched `health.connection` and passed happily while stopping Postgres
    for real produced a 500, because ATOMIC_REQUESTS opened a transaction before the view ran.
    Patching here exercises the middleware path too.
    """

    def unreachable(self: object) -> None:
        raise OperationalError("could not connect to server")

    monkeypatch.setattr(BaseDatabaseWrapper, "ensure_connection", unreachable)
